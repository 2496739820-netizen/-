...
test("server-renders verifiable Hupai Xiaohongshu work evidence", async () => {
  ...
  assert.match(html, /<span>粉丝<\/span><strong>3684粉丝<\/strong>/);
  assert.doesNotMatch(html, /<span>关注<\/span><strong>3684粉丝<\/strong>/);
  assert.match(html, /3684\s*粉丝/);
  assert.match(html, /1\.7\s*万获赞与收藏/);
  assert.match(html, /每日同步数据 · 最近更新：2026-08-14/);
  ...
  for (const metric of ["128", "150", "82", "90", "58", "30", "110", "72", "232", "196", "229", "242"]) {
...
test("server-renders an accessible dual-account Xiaohongshu evidence module", async () => {
  ...
  assert.match(html, /<span>粉丝<\/span><strong>3,334<\/strong>/);
  ...
  for (const metric of ["182,197", "8,066", "150,139", "6,864", "92,707", "3,108", "42,882", "1,642", "21,251", "849", "7,310", "369"]) {
    assert.match(html, new RegExp(`>${metric}<`));
  }
...
test("ships typed Hupai evidence data and complete original note covers", async () => {
  ...
  assert.equal(snapshot.snapshotDate, "2026-08-14");
  assert.equal(snapshot.hupai.followers, 3684);
...
test("ships six verified personal Xiaohongshu cards, covers, and tab behavior", async () => {
  ...
  assert.equal(snapshot.personal.works.montage.views, 182197);
